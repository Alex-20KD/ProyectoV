...
// Debe coincidir con los estados definidos en la entidad Pet
type NewStatus = 'Available' | 'PendingAdoption' | 'Adopted'
...
export class ChangePetStatusUseCase {
  ...
  async execute(request: ChangePetStatusRequest): Promise<Pet> {
    ...
    if (!actingUser || !actingUser.canManageAdoptions()) {
      throw new Error(
        'Permiso denegado. Solo administradores pueden cambiar el estado de la mascota.',
      )
    }
